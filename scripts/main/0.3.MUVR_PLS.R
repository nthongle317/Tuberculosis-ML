...
saveRDS(classModel, file=paste0(data_dir, "/data/train/DMP/TB-HC/MUVR_PLS_result.RDS"))

### Choose model (min, mid, max)
pdf(file=paste0(data_dir,"/data/train/DMP/TB-HC/MUVR_PLS_plot.pdf"), width=20, height=10)
...
# MUVR_res_min
write.table(MUVR_res_min, paste0(data_dir, "/data/train/DMP/TB-HC/PLS_min.txt"), col.names = T, row.names = F, quote = F, sep = "\t")

MUVR_res_mid = getVIP(classModel, model='mid')
# MUVR_res_mid
write.table(MUVR_res_mid, paste0(data_dir, "/data/train/DMP/TB-HC/PLS_mid.txt"), col.names = T, row.names = F, quote = F, sep = "\t")

MUVR_res_max = getVIP(classModel, model='max')
# MUVR_res_max
write.table(MUVR_res_max, paste0(data_dir, "/data/train/DMP/TB-HC/PLS_max.txt"), col.names = T, row.names = F, quote = F, sep = "\t")
