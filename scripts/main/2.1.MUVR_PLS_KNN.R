...
output_dir=paste0(directory, "/data/test/Model/MUVR_PLS")
...
SigCpG=read.table(paste0(directory, "/data/train/DMP/TB-HC/PLS_min.txt"), sep="\t", header=T)$name
...
scale <- preProcess(tuneDT[, -grep("Class", names(tuneDT))], method = "scale")
...
pdf(file.path(output_dir, "/KNN_Neighbor_CrossValidation.pdf"), width=3, height=3)
plot(knn.m1)
dev.off()

pre.m1s <- predict(knn.m1, tuneDT, type="prob")
...
pdf(file.path(output_dir, "/KNN_AUC_testDT.pdf"))
...
pre.m1s <- predict(knn.m1, valDT, type="prob")
...
pdf(file.path(output_dir, "/KNN_AUC_valDT.pdf"))
