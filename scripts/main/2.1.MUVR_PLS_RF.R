...
## the tuning Grid
rfGrid <- expand.grid(mtry = 1:(ncol(tuneDT)-1))
...
pdf(file.path(output_dir, "/RF_Predictors_CrossValidation.pdf"), width=3, height=3)
plot(rf.m1)
dev.off()
...
pdf(file.path(output_dir, "/RF_AUC_testDT.pdf"))
...
pdf(file.path(output_dir, "/RF_AUC_valDT.pdf"))
