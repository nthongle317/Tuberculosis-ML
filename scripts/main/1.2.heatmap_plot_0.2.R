...
sampleSheet=read.table(paste0(directory, "/config/train_data.tsv"),sep="\t", header=T)
cpgList=read.table(paste0(directory, "/data/train/DMP/TB-HC/DMPs.hg38.tsv"), sep="\t", header=T)

SigCpG=cpgList[abs(cpgList$diff)>=0.2,"probeID"]
...
png(file=file.path(paste0(output_dir, "/heatmap_0.2.png")), width=800, height=600)
print(heatmap_plot)
dev.off()

pdf(file=file.path(paste0(output_dir, "/heatmap_0.2.pdf")), width=8, height=7)
print(heatmap_plot)
dev.off()
# }
