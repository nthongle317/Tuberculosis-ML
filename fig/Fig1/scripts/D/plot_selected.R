...
### Get phenotype
pheno_type <- as.data.frame(fread(paste0(directory, "/config/train.tsv"), check.names=F, stringsAsFactors=F))
...
DMR_ID <- unique(DMRs_anno$dmrID)
  ...
  ht <- HighlightTrack(trackList = list(DMRtrack, DMPtrack, dtrack),
                       start = start-10, width = width+10,
                       chromosome = chr, col="black",
                       fill="transparent", lwd=0.5,
                       inBackground=FALSE)
  
  # Save it
  pdf(paste0(output_dir, "/", i_dmr, "_FullTrack.pdf"), width = 15, height = 10)
  plotTracks(list(itrack, gatrack, refGenes, genestrack, islandtrack, ht), from = start-2000, to = end+2000)
  dev.off()
}
