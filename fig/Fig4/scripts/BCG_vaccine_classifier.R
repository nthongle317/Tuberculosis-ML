...
TB_dt=TB[TB$probeID%in%SigCpG, names(TB)%in%c("probeID", pheno$Accession)]
...
label3 <- list(c("TB", "BCG vaccination"), c("TB", "HC"), c("BCG vaccination", "HC") )
...
ggsave(directory, "/fig/Fig4/figure/HC_BCG.plot.pdf", width=25, height=25)


# plot selected 
label3 <- list(c("TB", "BCG vaccination"), c("BCG vaccination", "HC") )

ggplot(dat[dat$probeID%in%c("cg21184174", "cg10140638", "cg10453758", "cg19616230", "cg00782174", "cg15705999"),], aes(x=Class, y=value, fill=Class)) + 
geom_violin()  +
stat_compare_means(comparisons = label3, size=7) +
facet_wrap(~probeID, scale="free", ncol = 2) + ylab("Beta value") +
theme(text = element_text(size = 30), legend.position = "none") 
ggsave(directory, "/fig/Fig4/figure/HC_BCG.selected.pdf", width=15, height=20)
ggsave(directory, "/fig/Fig4/figure/HC_BCG.selected.png", width=15, height=20)
